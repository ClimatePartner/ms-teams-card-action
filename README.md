...
![MS Teams Deploy Card](https://github.com/ClimatePartner/ms-teams-card-action/workflows/MS%20Teams%20Deploy%20Card/badge.svg)
...
jobs:
  build:
    ...
    steps:
      ...
      - uses: ClimatePartner/ms-teams-card-action@main #  or "./" if in a local set-up
        if: always()
        with:
...
2. Install JS dependencies via `pnpm install` or `npm install`.
3. Before pushing you changes, execute `pnpm ncc` (or `npm run ncc`) to create a build on `dist`.
...
```
  job
    |-- actions/checkout@v4
      |-- ms-teams-card-action (fires notification of job initiation, if allowed)
        |-- step 1
        |...more steps
        |-- step N
      |-- post ms-teams-card-action
